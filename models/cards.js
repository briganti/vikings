class Cards {
  ...
  constructor() {
    ...
    /* eslint-disable */
                  // Gold
    this.cards = [{ id: 0, type: 0, top: 4, right: 4, bottom: 7, left: 2, color: 0, req: [{ color: 1, lvl: 1 }, { color: 2, lvl: 1 }], provide: [] },
    ...
    this.starterDeckNbCards = { creatures: [5, 1, 1], tactics: 1, spells: 1 }
  }
  ...
  getMyStarterDeck() {
    ...
    for (let j = 0; j < this.starterDeckNbCards.tactics; j++) {
      index = Math.floor((Math.random() * (librairy.length)))
      starterDeck.push(librairy[index].id)
      librairy.splice(index, 1)
    }
    ...
    for (let j = 0; j < this.starterDeckNbCards.spells; j++) {
      index = Math.floor((Math.random() * (librairy.length)))
      starterDeck.push(librairy[index].id)
      librairy.splice(index, 1)
    }
  ...
  getARandomCard(pLibrary) {
    const cardsLn  = this.cards.length
    const cardsBag = []
    let currentCard

    // Player does not have all cards
    if (pLibrary.length < cardsLn) {
      for (let i = 0; i < cardsLn; i++) {
      ...
      if (cardsBag.length > 0) {
        const randNum = Math.floor((Math.random() * (cardsBag.length)))
